...
from disk.api.schema import UPDATE_DATE_FORMAT
...
def convert(value):
    ...
    обработки несериализуемых в JSON объектов. Функция должна вернуть либо
...
def convert_date(value: date):
    ...
    """
    return value.strftime(UPDATE_DATE_FORMAT)


@convert.register(Decimal)
def convert_decimal(value: Decimal):
    """
    asyncpg возвращает округленные перцентили возвращаются в виде экземпляров
    класса Decimal.
    """
    return float(value)
